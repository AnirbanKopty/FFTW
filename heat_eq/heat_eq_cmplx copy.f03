...
program heat_eq
    ...
    INTEGER, PARAMETER :: Nx = 64, Ny = 64              ! Nx = time discretization, Ny = Space
    REAL*8, PARAMETER :: L=2*PI                             
    REAL*8, PARAMETER :: dx = REAL(L)/REAL(Nx)
    ! INTEGER, PARAMETER :: max_itr = 64                 ! # of iteration
    ...
    planIFFT = fftw_plan_dft_2d(Ny,Nx, theta_hat, theta, FFTW_BACKWARD, FFTW_ESTIMATE)
    
    CALL fftw_execute_dft(planIFFT, theta_hat, theta)
